...
public interface GetEmployeeDetailsRepo extends JpaRepository<GetEmployeeDetails, Integer>{
 ...
 List<GetEmployeeDetails>  getEmpDetailListByBonusIdAssignedBonus(@Param("bonusId") int bonusId);
	...
	@Query(value = "SELECT\n" + 
			...
			"        sht.shiftname,\n" + 
			"        emptyp.name AS emp_type_name,\n" + 
			"        holidaycat.wo_cat_name AS sal_type_name,\n" + 
			"        'NA' AS fy_title      \n" + 
			"    FROM\n" + 
			"        m_employees emp      \n" + 
			"    LEFT JOIN\n" + 
			...
			"            ON     emp.current_shiftid = sht.id      \n" + 
			"    LEFT JOIN\n" + 
			"        weekoff_category holidaycat              \n" + 
			"            ON     holidaycat.wo_cat_id = emp.weekend_category      \n" + 
			"    WHERE\n" + 
			"        emp.del_status = 1          \n" + 
			"        AND emp.is_emp = 1", nativeQuery = true)
	List<GetEmployeeDetails> getAllEmplistForWeekoffCatAssign();
	...
	@Query(value = "SELECT\n" + 
			...
			"    salinfo.salary_type_id = saltype.sal_type_id\n" + 
			"WHERE\n" + 
			"    emp.del_status = 1 AND emp.is_emp = 1 AND emp.designation_id=:degnId", nativeQuery = true)
