...
public interface InfoForUploadAttendanceRepository extends JpaRepository<InfoForUploadAttendance, Integer>{
	...
	@Query(value="select COALESCE(( select count(*) from m_employees,tbl_emp_salary_info where m_employees.emp_id=tbl_emp_salary_info.emp_id),0)  as total_emp, DATEDIFF(:toDate, :fromDate) as date_diff, "
	...
	InfoForUploadAttendance getInformationOfUploadedAttendance(@Param("fromDate") String fromDate, @Param("toDate") String toDate);
